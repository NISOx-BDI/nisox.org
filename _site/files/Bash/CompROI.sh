...
SetUp() {
    ...
    # Some error checking
    if [ $(${FSLDIR}/bin/imtest "$ROI") = 0 ] ; then 
	echo "ERROR: ROI image '$ROI' doesn't exist"; ok=0;
    fi
    if [ ${#IDs[*]} -ne ${#Imgs[*]} ] ; then 
...
SetROIdef() {
	...
	if [ `awk -F, '{if (NR==2) print $2}' "$ROIdef"` != "" ] ; then
...
# Create ROI masks
echo -n "Creating nROI=$nROI mask images..."
MkROImsks
echo "."
